...
font_access_test(async t => {
  ...
  assert_greater_than_equal(
    fonts.length, 1,
    'All available fonts should be returned even when an empty object is '
      + 'passed for input.');
}, 'query(): empty object');
...
font_access_test(async t => {
  ...
  assert_greater_than_equal(
    fonts.length, 1,
    'All available fonts should be returned even when an empty array of select '
      + 'is passed for input.');
...
font_access_test(async t => {
  ...
  assert_greater_than_equal(
    fonts.length, 1,
    'All available fonts should be returned even when undefined is passed for '
    + 'input.');
...
for (const test of non_ascii_input) {
  font_access_test(async t => {
    ...
    assert_equals(
        fonts.length, 0,
        `Fonts should not be selected for ascii character input: '
          + '${JSON.stringify(fonts)}`);
  }, `query(): ascii character input: ${JSON.stringify(test)}`);
}